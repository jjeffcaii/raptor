      ...
      is CommandResult -> {
        ...
        when (code) {
          "NetConnection.Connect.Success" -> {
            ...
            b = payload.toBuffer()
            header = Header(FMT.F1, msg.header.csid, 0L, 0L, ChunkType.COMMAND_AMF0, b.length())
            backend.write(Buffer.buffer().appendBuffer(header.toBuffer()).appendBuffer(b))
            if (logger.isDebugEnabled) {
            ...
            when (cmd.transId) {
              this.tidOfCreateStream -> {
                ...
                val header = Header(FMT.F0, msg.header.csid + 1, 0L, 1L, ChunkType.COMMAND_AMF0, b.length())
                backend.write(Buffer.buffer().appendBuffer(header.toBuffer()).appendBuffer(b))
              }
              else -> {
                //TODO
